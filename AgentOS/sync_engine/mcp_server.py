...
async def list_tools() -> list[Tool]:
    return [
        ...
        # ── Peer Mesh (3-way: Zo + Antigravity + AgentOS) ──
        ...
        Tool(name="get_peers",
             description="List all registered peers in the 3-way mesh (Zo, Antigravity, AgentOS) and their status.",
             inputSchema={"type": "object", "properties": {
                 "agent_id": {"type": "string"}
             }, "required": ["agent_id"]}),
        ...
        Tool(name="get_session",
             ...
             inputSchema={"type": "object", "properties": {
                 ...
                 "target_agent": {"type": "string", "description": "Agent whose session to read (zo | antigravity | agentos | all)"}
             }, "required": ["agent_id"]}),
        ...
        Tool(name="call_peer_tool",
             ...
             inputSchema={"type": "object", "properties": {
                 "agent_id": {"type": "string"},
                 "target_agent": {"type": "string", "description": "zo | antigravity | agentos"},
                 "tool_name": {"type": "string"},
                 "arguments": {"type": "object", "default": {}}
             }, "required": ["agent_id", "target_agent", "tool_name"]}),

        # ── AgentOS workforce tools (proxied via peer bridge) ──
        Tool(name="agentos_invoke_agent",
             description="Invoke a named AgentOS workforce agent (e.g. Taylor, Alex) to perform a task.",
             inputSchema={"type": "object", "properties": {
                 "agent_id": {"type": "string"},
                 "agent_name": {"type": "string", "description": "AgentOS agent name"},
                 "task": {"type": "string"},
                 "context": {"type": "object", "default": {}}
             }, "required": ["agent_id", "agent_name", "task"]}),

        Tool(name="agentos_assign_task",
        ...
        Tool(name="agentos_query_memory",
        ...
        Tool(name="agentos_war_room",
             description="Get the current AgentOS War Room state (active projects, blockers, metrics).",
             inputSchema={"type": "object", "properties": {
                 "agent_id": {"type": "string"}
             }, "required": ["agent_id"]}),
        ...
        Tool(name="ide_open_file",
             description="Open a file in Antigravity IDE. AgentOS and Zo can trigger this.",
             inputSchema={"type": "object", "properties": {
                 "agent_id": {"type": "string"},
                 "path": {"type": "string"}
             }, "required": ["agent_id", "path"]}),
...
async def call_tool(name: str, arguments: dict) -> list[TextContent]:
    ...
    # ── AgentOS workforce (proxied via peer bridge) ──
    elif name in ("agentos_invoke_agent", "agentos_assign_task",
                  "agentos_query_memory", "agentos_war_room"):
        result = await agentos_client.dispatch_agentos_tool(name, arguments, caller=agent_id)
        return ok(result)

    # ── Antigravity IDE tools (called by AgentOS / Zo) ──
